# Home / ទំព័រដើម

Welcome to the official documentation for **DevSolve** — a modern Bug Bounty and Vulnerability Coordination Platform connecting Security Researchers, Developers, and Organizations.

សូមស្វាគមន៍មកកាន់មជ្ឈមណ្ឌលឯកសារផ្លូវការរបស់ **DevSolve** — វេទិកា Bug Bounty និងគ្រប់គ្រងចន្លោះប្រហោងសុវត្ថិភាព ដែលតភ្ជាប់រវាងអ្នកស្រាវជ្រាវសុវត្ថិភាព (Security Researchers), អ្នកអភិវឌ្ឍន៍ (Developers) និងក្រុមហ៊ុន/ស្ថាប័ន (Organizations)។

***

## 🌐 Choose Your Language / សូមជ្រើសរើសភាសា

| 🇰🇭 ភាសាខ្មែរ (Khmer) | 🇬🇧 English |
| ------------------------------------------- | ----------------------------------------------------- |
| អានឯកសារណែនាំ និងរបៀបប្រើប្រាស់ជាភាសាខ្មែរ | Read documentation and platform guides in English |
| 👉 [**ចូលអានជាភាសាខ្មែរ**](khmer-documentation/km.md) | 👉 [**Read in English**](english-documentation/en.md) |

***

## 🚀 Quick Navigation / ផ្លូវកាត់សំខាន់ៗ

### 🇰🇭 ភាសាខ្មែរ

* [**ការចាប់ផ្តើមដំបូង**](khmer-documentation/overview/) — ស្វែងយល់ពី DevSolve និងរបៀបចុះឈ្មោះ
* [**សម្រាប់ Researchers & Developers**](khmer-documentation/finding-programs/) — របៀបស្វែងរក Program និង Submit Report
* [**សម្រាប់ក្រុមហ៊ុន (Organizations)**](khmer-documentation/registration/) — របៀបបង្កើត Program និងគ្រប់គ្រង Report
* [**គោលការណ៍សុវត្ថិភាព (Rules & Policies)**](khmer-documentation/responsible-disclosure/) — Safe Harbor និងក្រមសីលធម៌
* [**សំណួរដែលសួរញឹកញាប់ (FAQ)**](khmer-documentation/reports-faq/) — ចម្លើយចំពោះចម្ងល់នានា

### 🇬🇧 English

* [**Getting Started**](english-documentation/overview/) — Overview and account onboarding
* [**For Researchers & Developers**](english-documentation/finding-programs/) — Finding bounty programs and submitting vulnerability reports
* [**For Organizations**](english-documentation/registration/) — Launching bug bounty programs and triaging reports
* [**Safety & Policies**](english-documentation/responsible-disclosure/) — Safe Harbor and disclosure rules
* [**Frequently Asked Questions (FAQ)**](english-documentation/reports-faq/) — Common questions and troubleshooting
